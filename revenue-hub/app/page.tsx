...
const SEED_PROJECTS: Array<{
  ...
  {
    title: 'Autosphere Imports',
    category: 'Website',
    description: 'Modern automobile imports website for Autosphere Imports with a full product catalogue, vehicle listings, enquiry system, and elegant design tailored for a Ghana-based car dealership.',
    image: '',
    features: ['Vehicle catalogue & listings', 'Enquiry & contact system', 'Responsive mobile-first design', 'Clean product showcase UI', 'Brand-consistent styling'],
    technologies: ['HTML5', 'CSS3', 'JavaScript'],
    link: 'https://github.com/PlnDominic/Autosphere-Imports',
  ...
  },
  {
    ...
    link: 'https://github.com/PlnDominic/Gusty-Women-Foundation',
    year: 2025, status: 'completed',
  },
  {
    ...
    link: 'https://github.com/PlnDominic/Local-Drop-Shipping',
    year: 2025, status: 'completed',
  },
  {
    title: 'Raynelle',
    ...
    technologies: ['JavaScript', 'HTML5', 'CSS3'],
    link: 'https://github.com/PlnDominic/Raynelle',
    year: 2025, status: 'completed',
  },
  {
    title: 'Aspee',
    ...
    link: 'https://github.com/PlnDominic/Aspee',
    year: 2025, status: 'completed',
  },
  {
    title: 'Glow Healthy',
    ...
    link: 'https://github.com/PlnDominic/glow-healthy',
    year: 2025, status: 'completed',
  },
]
    ...
    try {
      const res = await fetch('/api/chat', {
        ...
        body: JSON.stringify(isReferralAsk ? {
          ...
          systemPrompt: 'You are ContentBot writing on behalf of Dominic, the CEO of Ecstasy Technologies, a web design and development company based in Bibiani, Ghana. Write a short WhatsApp follow-up to a prospect who has not replied to the first pitch. It must NOT repeat or rephrase the original pitch — bring something new: a *different* pain point common to businesses in their industry (lost bookings, invisible to Google search, no way to show pricing/menu/portfolio without a back-and-forth chat, losing customers to a competitor who already has a site, etc.) and a concrete way a website removes it, or a relevant proof point from similar work. Output only the message body — no subject line, no labels.',
          messages: [{ role: 'user', content: `Write follow-up touch ${sequenceStep + 1} to this prospect (no reply so far):\n\nBusiness: ${deal.name}\nIndustry: ${deal.industry || 'unknown'}\nEstimated value: GHS ${deal.valueGHS.toLocaleString()}\n\n2-3 sentences, a genuinely different pain point than a first cold-open would use, tied to what a "${deal.industry || 'business'}" in Ghana concretely loses by not having a good website, one soft CTA. Sign off as Dominic.` }],
        } : {
          systemPrompt: 'You are ContentBot writing on behalf of Dominic, the CEO of Ecstasy Technologies, a web design and development company based in Bibiani, Ghana. Write concise, friendly WhatsApp messages for prospect outreach. Every pitch must be built around ONE specific, believable pain point that businesses in the prospect\'s exact industry face day to day (e.g. a restaurant losing walk-ins because people can\'t find hours/menu/location online, a boutique stuck selling only through Instagram DMs with no way to show a full catalogue or take orders, a clinic losing bookings to phone tag, a construction firm losing bids because they have no portfolio to show proof of past work, a school parents can\'t find or vet online). Name the pain in plain, concrete terms — never generic ("get online", "grow your business") — then say specifically how a website solves that exact pain. Output only the message body — no subject line, no labels.',
  ...
  return (
    ...
    >
      ...
      <div style={{ display: 'flex', gap: 4, marginTop: 8, flexWrap: 'wrap' }}>
        <button onClick={() => onOpenAgent('content', `Write a WhatsApp pitch for:\nBusiness: ${deal.name}\nIndustry: ${deal.industry}\nValue: GHS ${deal.valueGHS}\nStage: ${STAGE_LABELS[deal.stage]}\n\nIdentify the single most concrete, believable pain point a "${deal.industry || 'business'}" like this has because it lacks a good website (be specific to the industry, not generic "get online" language), then build the pitch around how a website solves exactly that pain.`)} style={{ fontSize: 10, padding: '3px 7px', borderRadius: 10, border: `1px solid ${BORDER}`, background: 'transparent', color: MUTED, fontFamily: FONT_BODY, cursor: 'pointer' }}>
          Pitch
        </button>
